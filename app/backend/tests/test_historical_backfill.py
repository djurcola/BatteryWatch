...
class HistoricalBackfillTests(unittest.TestCase):
    def test_ensures_claims_dispatches_and_finalizes_supplied_plan(self) -> None:
        ...
        claims = [scada_claim, price_claim, None]
        assets = (
            BatteryAsset(
                "BAT1",
                "Battery One",
        ...
        progress = BackfillRunProgress("run-1", "completed", 2, 0, 0, 2, 0, 2)
        ...
        class Ledger:
            ...
            def ensure_run(self, actual_spec: BackfillRunSpec, items: Any) -> BackfillEnsureResult:
                ...
                self.assert_equal(tuple(items), (scada_item, price_item))
                operations.append(f"ensure:{self.connection.number}")
                return BackfillEnsureResult(True, False, 2, 0)
        ...
        self.assertEqual(result.ensure_result, BackfillEnsureResult(True, False, 2, 0))
        self.assertEqual(result.claims, (scada_claim, price_claim))
        ...
        self.assertEqual(result.replayed_interval_count, 1)
        self.assertEqual(result.replayed_outer_artifact_count, 1)
        self.assertEqual(
            operations,
            ["ensure:1", "claim:2", "scada", "claim:3", "price", "claim:4", "finalize:5"],
        )
        self.assertEqual([connection.close_calls for connection in connections], [1, 1, 1, 1, 1])
