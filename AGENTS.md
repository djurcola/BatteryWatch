...
- Bitwarden Secrets Manager is the credential authority. The current Coder service account has read/write access, but every read or write still requires an approved, narrowly scoped secret-handling task and non-disclosing verification.
- The explicit host keys are `192.168.20.42_USER` and `192.168.20.42_SUDO_PASSWORD`. The legacy `SUDO_PASSWORD` key may remain for compatibility; do not silently rotate or delete it when creating clearer aliases.
...
2. Create a release tarball that excludes `.git`, caches, local env files, and credentials. It must include `app/backend`, `app/config`, `app/deploy`, and `app/migrations`; `app/frontend/dist` is untracked and must be built and copied into the candidate explicitly.
3. Copy it to `192.168.20.42`, extract into a new immutable `$HOME/batterywatch/releases/<release-id>`, install/update that release's venv as required, and include the verified frontend build. Run the complete backend suite inside the candidate before flipping `current`; migration-presence tests deliberately catch incomplete packages.
...
- CURRENT and ARCHIVE can contain the same interval artifact under the same `source_artifact_id` but different official URLs. Treat this as an idempotent replay only when filename, CSV member, report timestamp, SHA-256, and raw ZIP bytes also match exactly. A mismatch in any evidence field remains an immutable conflict. The daily outer archive still preserves archive provenance.
- Resume an interrupted run with the same run ID only when start, end, ingestion version, and planned items are unchanged. `ensure_run` recovers a stale `running` item while retaining completed dates. Use a new run ID when the plan changes; never edit ledger rows to manufacture a clean retry.
- This branch's live service still calls `collect_latest_dispatch_scada`; `collector_service --once` replays only the latest file and does not enumerate every unseen CURRENT-index artifact. A still-listed skipped file can be recovered only by selecting its canonical `DispatchScadaArtifactRef` and passing the verified collection through `run_collection_cycle`, or by waiting for the daily archive. The newer per-file ledger/catch-up implementation lives in a separate unintegrated collector lane; do not assume it is active from its existence elsewhere.
...
- Do not delete raw artifacts, ledger evidence, or failed runs to make a retry appear clean. Resume an identical plan or use a new run ID for a changed plan, and retain all provenance.
